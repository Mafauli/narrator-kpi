-- Create enum for business model (if not exists)
DO $$ BEGIN
...
-- Create enum for tone (if not exists)
DO $$ BEGIN
...
-- Create enum for email status (if not exists)
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS public.connections_airtable (
...
CREATE TABLE IF NOT EXISTS public.airtable_views (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
DROP TRIGGER IF EXISTS update_connections_airtable_updated_at ON public.connections_airtable;
CREATE TRIGGER update_connections_airtable_updated_at
  BEFORE UPDATE ON public.connections_airtable
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_airtable_views_updated_at ON public.airtable_views;
